...
import { Species, type SurgeryCase, TaskType } from "../../../backend";
...
export function CalendarCaseCard({
  surgeryCase,
  onNavigateToCase,
}: CalendarCaseCardProps) {
  ...
  const allCompleted =
    ...
    selectedItems.every(
      (item) => surgeryCase.task[item.completedField] === true,
    );
  ...
  return (
    <button
      ...
      className={`w-full text-left rounded border bg-card p-2 shadow-sm transition-all cursor-pointer hover:shadow-md hover:border-primary/40 hover:bg-accent/30 ${
